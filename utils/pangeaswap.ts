...
function encodeCalculateExactInput(
  path: string[],
  tokenIn: string,
  amount: BigNumberish
) {
  return swapHelperInterface.encodeFunctionData("calculateExactInput", [
    path,
    tokenIn,
    amount,
  ]);
...
export class PangeaswapFetcher implements IFetcher {
  private pools: Pool[] = [];
  ...
  async swap(
    ...
    const targetPools = this.pools.filter((pool) =>
      includeTokens(pool, tokenSets)
    );
    ...
    try {
      ...
      const estimatedOutputs = (
        await this.multicall.callStatic.multicall(calldata)
      ).returnData
        .map((e) => e.returnData)
        .map((e) =>
          swapHelperInterface.decodeFunctionResult("calculateExactInput", e)
        )
        .map((e) => e[0]);
      ...
      fromToken = addressEqual(fromToken, this.wklayAddress)
        ? ethers.constants.AddressZero
        : fromToken;
      const data = encodeExactInput(
        fromToken,
        route,
        amount,
        estimatedAmount.mul(99).div(100),
        fromAddress
      );
      return {
        estimatedAmount,
        ...
        data,
        value: addressEqual(fromToken, this.wklayAddress)
...
function computeAllRoutes(
  ...
  for (const pool of pools) {
    ...
    const outputToken = addressEqual(pool.token0, tokenIn)
      ? pool.token1
      : pool.token0;
...
function includeTokens(pool: Pool, tokens: Set<string>) {
  return (
    tokens.has(pool.token0.toLowerCase()) ||
    tokens.has(pool.token1.toLowerCase())
  );
}
